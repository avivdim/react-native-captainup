...
const LINKING_ERROR = `The package 'react-native-captainup' doesn't seem to be linked. Make sure: \n\n` + Platform.select({
  ios: "- You have run 'pod install'\n",
  default: ''
}) + '- You rebuilt the app after installing the package\n' + '- You are not using Expo Go\n';
const CaptainupNative = NativeModules.Captainup ? NativeModules.Captainup : new Proxy({}, {
  get() {
 ...
 */
export function initialize(apiKey, mobileToken) {
 ...
 */
export function authenticate(userId, userName) {
 ...
 */
export function sendAction(actionName) {
 ...
 */
export function onBadgesAchieved(callback) {
 ...
 * Call .remove() on the returned subscription to unsubscribe.
 */
export function onLevelUpdate(callback) {
